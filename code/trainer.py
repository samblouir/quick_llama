...
def chat_tokenizer(x, system_prompt=None,):
	msgs = []
...
def prep_ds(ds, config, split=None,):
	...
	try:
		...
		for question_idx, (question) in enumerate(ds):
			...
			# formatted_input = format_multiple_choice(**question)
			# input_ids = chat_tokenizer(formatted_input['input'])
			# label_ids = np.int64(tokenizer(formatted_input['label']).tolist() + [_tokenizer.eos_token_id])
			...
			if config.get("dataset", None) in ['hermes']:
				...
				# accelerator.print(f"  lid: {lid}")
				# print(f"  lid: {lid}", flush=True)
				label_ids = np.int64(lid + [_tokenizer.eos_token_id])
				# accelerator.print(f"  input_ids.shape: {input_ids.shape}, label_ids.shape: {label_ids.shape}")
...
def find_and_kill_accelerate_processes():
    ...
    for proc in psutil.process_iter(['pid', 'name', 'cmdline']):
        try:
            pinfo = proc.info
            cmdline = pinfo['cmdline'] # Get the command line as a list of arguments

            # Check if cmdline is not empty and if 'accelerate' is in any argument
            # Using lower() makes the check case-insensitive
            if cmdline and any('accelerate' in arg.lower() for arg in cmdline):
                pid = pinfo['pid']
                # Important: Don't target the script itself if it was run like
                # python your_script.py accelerate_arg
                if pid == os.getpid():
                    continue

                # Join the command line arguments for display
                command_str = ' '.join(cmdline)
                print(f"  Found 'accelerate' process: PID={pid}, Command='{command_str}'")
                target_processes.append(proc) # Store the process object

        except (psutil.NoSuchProcess, psutil.AccessDenied, psutil.ZombieProcess):
            # Process might have terminated, or we don't have permission to access it
            pass
        except Exception as e:
            # Catch other potential errors accessing process info
    ...
    for proc in target_processes:
        try:
            ...
            # Optional: Wait a moment and check if it terminated, then force kill
            # try:
            #     proc.wait(timeout=3) # Wait up to 3 seconds
            #     print(f"    PID {pid_to_kill} terminated successfully.")
            # except psutil.TimeoutExpired:
            #     print(f"    PID {pid_to_kill} did not terminate gracefully after SIGTERM. Sending SIGKILL...")
            #     proc.kill() # Send SIGKILL (forceful termination)
            #     print(f"    Sent SIGKILL to PID {pid_to_kill}.")

        except psutil.NoSuchProcess:
            # Process already terminated between finding and killing
            ...
            # Catch other potential errors during termination
...
def main():
	...
	# softmax_temperatures = [0.2, 0.4, 0.6, 0.8, 0.9, 1.0, 1.1, 1.2, 1.4, 1.6, 1.8, 2.0, 3.0, 5.0,]
	...
	# ds = load_dataset("lighteval/mmlu", "machine_learning",)
	...
	# 	ds = ds.map(lambda x: {value: x[key]}, remove_columns=[key],)

	# ds_train = ds['dev']
	# ds_train = ds['auxiliary_train']
	...
	# run_evals(config_name=config_hash)
	# exit()
	...
	for step in range(num_training_steps):
		...
		if (step % config['steps_between_evals'] == 0) or (step == num_training_steps - 1):# or (step == num_training_steps - 4):
			accelerator.wait_for_everyone()
			eval_loss = run_eval()
			accelerator.wait_for_everyone()
		...
		if ((num_training_steps - 3) < step):
			...
			# our_pid = os.getpid()
			# try:
			# 	os.kill(our_pid, signal.SIGINT)
			# 	os.kill(our_pid, signal.SIGKILL)
			# except Exception as e:
			# 	accelerator.print(f"  Failed to kill process: {e}")
			# 	os._exit(1)
			# sys.exit(1)
